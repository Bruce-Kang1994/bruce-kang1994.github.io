...
> **阅读定位**：这是 [产品增长通用框架](/lab/posts/growth-dimension-framework/) Step 1（用户研究）的方法论补充。通用框架说"先做用户研究"，本文给你一个具体的量化工具——怎么用 4 个问题测出你离 PMF 有多远，以及怎么系统性地缩小差距。也和 [ElevenLabs 访谈](/lab/posts/elevenlabs-interview-highlights/) 中 Luke 说的"PMF 前别做 paid"直接关联——不是说你"感觉有 PMF"就行，要能量出来。
...
不是。PMF 是一个连续的分数——你可以有 20% 的 PMF，也可以有 60% 的 PMF。而且分数会变——今天有 PMF 不代表明天还有（参考 [Reforge 增长体系](/lab/posts/reforge-growth-frameworks/) 中 Balfour 关于 AI 时代 PMF 波动性的警告）。
...
**前提条件**：产品需要到"可用"状态，用户能完成核心流程。在产品还是 demo 的阶段，这个方法还用不了——先把 [产品增长通用框架](/lab/posts/growth-dimension-framework/) Step 1 的定性用户研究做好。
...
| [产品增长通用框架](/lab/posts/growth-dimension-framework/) | Step 1 用户研究给你定性理解，四问调查给你定量验证。定性先行，定量跟上 |
